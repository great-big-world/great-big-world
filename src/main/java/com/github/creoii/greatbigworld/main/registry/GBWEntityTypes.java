...
public class GBWEntityTypes implements Register {
    ...
    public void register() {
        ...
        registerEntity(NAUTILUS, new Identifier(GreatBigWorld.NAMESPACE, "nautilus"), new EntitySettings<>(NAUTILUS, BiomeSelectors.tag(Tags.BiomeTags.NAUTILUS_SPAWNABLE), SpawnGroup.WATER_CREATURE, 42, 1, 3, 13418935, 16748893, Items.MULE_SPAWN_EGG));
    }
    ...
    public record EntitySettings<L extends MobEntity, T extends EntityType<L>>(T entityType, Predicate<BiomeSelectionContext> biomeSelector, SpawnGroup group, int weight, int minGroupSize, int maxGroupSize, int primaryEggColor, int secondaryEggColor, Item after) {
        public void register() {
            ...
            GBWItems.registerItem(new Identifier(GreatBigWorld.NAMESPACE, Registry.ENTITY_TYPE.getId(entityType).getPath() + "_spawn_egg"), new SpawnEggItem(entityType, primaryEggColor, secondaryEggColor, new FabricItemSettings().group(ItemGroup.MISC)) {
                @Override
