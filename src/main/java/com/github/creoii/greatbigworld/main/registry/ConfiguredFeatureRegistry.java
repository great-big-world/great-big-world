...
public class ConfiguredFeatureRegistry implements Register {
    ...
    public void register() {
        MAHOGANY = BuiltinRegistries.add(BuiltinRegistries.CONFIGURED_FEATURE, new Identifier(NAMESPACE, "mahogany"),
                new ConfiguredFeature<>(Feature.TREE, new TreeFeatureConfig.Builder(SimpleBlockStateProvider.of(BlockRegistry.MAHOGANY.log().getDefaultState()), new TwistingTrunkPlacer(3, 2, 0, UniformIntProvider.create(4, 6), UniformIntProvider.create(1, 2), UniformIntProvider.create(2, 3), UniformIntProvider.create(1, 2), false), SimpleBlockStateProvider.of(BlockRegistry.MAHOGANY_LEAVES), new AcaciaFoliagePlacer(ConstantIntProvider.create(3), ConstantIntProvider.create(1)), new TwoLayersFeatureSize(9, 2, 6)).dirtProvider(SimpleBlockStateProvider.of(Blocks.DIRT)).build())
        );
        GREEN_ASPEN = BuiltinRegistries.add(BuiltinRegistries.CONFIGURED_FEATURE, new Identifier(NAMESPACE, "green_aspen"),
                new ConfiguredFeature<>(Feature.TREE, new TreeFeatureConfig.Builder(SimpleBlockStateProvider.of(BlockRegistry.ASPEN.log().getDefaultState()), new StraightTrunkPlacer(6, 3, 1), SimpleBlockStateProvider.of(BlockRegistry.GREEN_ASPEN_LEAVES), new AspenFoliagePlacer(UniformIntProvider.create(2, 3), ConstantIntProvider.create(2), UniformIntProvider.create(2, 3)), new TwoLayersFeatureSize(2, 0, 1)).decorators(List.of(new BranchTreeDecorator(BlockRegistry.ASPEN.log().getDefaultState(), .2f, false, "minecraft:leaves", UniformIntProvider.create(2, 6), true))).build())
        );
        YELLOW_ASPEN = BuiltinRegistries.add(BuiltinRegistries.CONFIGURED_FEATURE, new Identifier(NAMESPACE, "yellow_aspen"),
                new ConfiguredFeature<>(Feature.TREE, new TreeFeatureConfig.Builder(SimpleBlockStateProvider.of(BlockRegistry.ASPEN.log().getDefaultState()), new StraightTrunkPlacer(6, 3, 1), SimpleBlockStateProvider.of(BlockRegistry.YELLOW_ASPEN_LEAVES), new AspenFoliagePlacer(UniformIntProvider.create(2, 3), ConstantIntProvider.create(2), UniformIntProvider.create(2, 3)), new TwoLayersFeatureSize(2, 0, 1)).decorators(List.of(new BranchTreeDecorator(BlockRegistry.ASPEN.log().getDefaultState(), .2f, false, "minecraft:leaves", UniformIntProvider.create(2, 6), true))).build())
        );
