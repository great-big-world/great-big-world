...
public class RandomSpreadFeature extends Feature<RandomSpreadFeatureConfig> {
    ...
    public boolean generate(FeatureContext<RandomSpreadFeatureConfig> context) {
        ...
        for (BlockPos pos : GenerationUtil.randomWalk(context.getOrigin(), config.iterations().get(GreatBigWorld.RANDOM), config.walkLength().get(GreatBigWorld.RANDOM), config.randomStart(), config.randomStartRadius().get(0), config.randomStartRadius().get(1))) {
